...
#>
param(
    [string]$InstallDir = "build/ANYRAID-ORCA",
    [string]$OutputPath = "build/ANYRAID-ORCA_Windows_MSIX.msix",
    [ValidateSet("x64", "arm64")]
    [string]$Architecture = "x64",
    [string]$StagingDir = "",
    [switch]$StageOnly,
    [string]$IdentityName = "ANYRAID.ANYRAIDORCA",
    [string]$Publisher = "CN=ANYRAID",
    [string]$PublisherDisplayName = "ANYRAID"
)
...
if (-not (Test-Path (Join-Path $InstallDir 'anyraid-orca.exe'))) {
    throw "anyraid-orca.exe not found in '$InstallDir' - build the install tree first"
}

if ([string]::IsNullOrEmpty($StagingDir)) {
    $StagingDir = Join-Path ([System.IO.Path]::GetTempPath()) 'anyraid-orca-msix-staging'
}
