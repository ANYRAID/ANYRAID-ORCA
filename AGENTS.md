...
- For translation changes (`localization/i18n/**/*.po`), check that recurring terms stay consistent within that language.

## Localization & translations

Catalogs live in `localization/i18n/<lang>/OrcaSlicer_<lang>.po`; the template is `OrcaSlicer.pot`.
Use the localization files and repository conventions as the source of truth for these principles.

### Terminology

- Keep recurring terms consistent within each language, and do not translate brand/product names, acronyms, materials, file formats, G-code tokens, macros, variables, or identifiers that must stay in English.
